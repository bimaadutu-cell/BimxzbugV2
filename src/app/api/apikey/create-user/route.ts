...
export async function POST(req: Request) {
  ...
  if (!validRoles.includes(role)) return Response.json({ ok: false, message: "Role tidak valid." }, { status: 400 });
  ...
  const [newUser] = await db.insert(users).values({ username: uname, password: String(password), role, expiresAt, isActive: true }).returning();
  return Response.json({
    ok: true,
    message: `Akun ${role} berhasil dibuat via APIKEY. Berlaku ${days ? days + " hari" : "seumur hidup"} — otomatis terhapus saat expired.`,
    user: { id: newUser.id, username: newUser.username, role: newUser.role, expiresAt: newUser.expiresAt, createdAt: newUser.createdAt },
    expiresInDays: days,
